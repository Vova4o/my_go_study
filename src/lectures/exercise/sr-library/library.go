...
func checkOutBook(library *Library, title Title, member *Members) bool {
	...
	if book.lended == book.total {
		fmt.Println("No more books avalable to lend")
		return false
	}
...
func main() {
	...
	library.members["Jayson"] = Members{"Jayson", make(map[Title]LendAudit)}
	library.members["Helper"] = Members{"Helper", make(map[Title]LendAudit)}
	library.members["Duper"] = Members{"Duper", make(map[Title]LendAudit)}

	fmt.Println("\nInitial:")
	printLibraryBooks(&library)
	printMemberAudits(&library)

	member := library.members["Duper"]
